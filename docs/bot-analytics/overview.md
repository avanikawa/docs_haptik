...
- The Conversation Split metrics will help you understand which conversations are being handed over to live support.
- It could be used to uncover root causes of deflection to the call center or live chat tool.
- With this information, you can identify opportunities for further automation, allowing customers to skip the agent and instead self-serve to get their answers by automating top asked queries, finding new data sources or training sets for improving the performance.
- Agent Handover is not in control of the bot, hence need to come to a benchmark for bot automation where agent handover take place by design.
...
* **Agent Handover by Design**: This is the percentage of conversations where the is agent handover design in the conversation flow on the bot.
